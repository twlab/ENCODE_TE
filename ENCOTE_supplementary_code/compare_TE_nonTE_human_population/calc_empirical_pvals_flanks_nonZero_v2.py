...
Version 2 combines right and left flanks together for empirical p-value calculation and null distribution.
Usage: python3 calc_empirical_pvals_flanks_nonZero_v2.py <observed varFreq file> <observed left flank varFreq file> <observed right flank varFreq file> 
...
with open(sys.argv[-1] + '_shuffled_TE_mean', 'w') as o:
	o.write('Shuffle_num\tcCRE_type\tDifference\n')
	for ccre in list_ccres:
		...
		observed_mean_diff = stats.mean([observed_mean_diff_left, observed_mean_diff_right])
		...
		for i in sorted(shuffled_stats['TE'][ccre]['mean']):
			...
			o.write(str(i) + '\t' + ccre + '\t' + str(shuffled_diff_left) + '\n')
			o.write(str(i) + '\t' + ccre + '\t' + str(shuffled_diff_right) + '\n')
		counter = 0
		for i in range(len(list_shuffled_diff_left)):
			if observed_mean_diff < list_shuffled_diff_left[i]:
				counter += 1
		for i in range(len(list_shuffled_diff_right)):
			if observed_mean_diff < list_shuffled_diff_right[i]:
				counter += 1
		pval = counter/(len(list_shuffled_diff_right) + len(list_shuffled_diff_left))
		results['TE_mean'][ccre] = [pval, observed_mean_diff, stats.mean(list_shuffled_diff_right + list_shuffled_diff_left)]
results['nonTE_mean'] = {}
	...
	for ccre in list_ccres:
		...
		for i in range(len(list_shuffled_diff_right)):
			if observed_mean_diff < list_shuffled_diff_right[i]:
				counter += 1
		pval = counter/(len(list_shuffled_diff_right) + len(list_shuffled_diff_left))
		results['nonTE_mean'][ccre] = [pval, observed_mean_diff, stats.mean(list_shuffled_diff_right + list_shuffled_diff_left)]
...
with open(sys.argv[-1] + '_shuffled_TE_perc', 'w') as o:
	...
	for ccre in list_ccres:
		...
		observed_perc_diff = stats.mean([observed_perc_diff_left, observed_perc_diff_right])
		list_shuffled_diff_left = []
		list_shuffled_diff_right = []
		for i in sorted(shuffled_stats['TE'][ccre]['perc']):
		...
		counter = 0
		for i in range(len(list_shuffled_diff_left)):
			if observed_perc_diff < list_shuffled_diff_left[i]:
				counter += 1
		for i in range(len(list_shuffled_diff_right)):
			if observed_perc_diff < list_shuffled_diff_right[i]:
				counter += 1
		pval = counter/(len(list_shuffled_diff_right) + len(list_shuffled_diff_left))
		results['TE_perc'][ccre] = [pval, observed_perc_diff, stats.mean(list_shuffled_diff_right + list_shuffled_diff_left)]
results['nonTE_perc'] = {}
	...
	for ccre in list_ccres:
		...
		for i in range(len(list_shuffled_diff_right)):
			if observed_perc_diff < list_shuffled_diff_right[i]:
				counter += 1
		pval = counter/(len(list_shuffled_diff_right) + len(list_shuffled_diff_left))
		results['nonTE_perc'][ccre] = [pval, observed_perc_diff, stats.mean(list_shuffled_diff_right + list_shuffled_diff_left)]
