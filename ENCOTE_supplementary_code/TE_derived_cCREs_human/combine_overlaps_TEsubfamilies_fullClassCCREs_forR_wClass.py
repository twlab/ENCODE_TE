'''
Combines number of overlaps for TE subfamilies in each cCRE type across full classified cell/tissue types.
Outputs each cCRE into separate files
Takes input for the minimum number of overlaps required to report overlap number
TE subfamily is only reported if at least one cCRE type + cell/tissue type combination passes the overlap threshold
Usage: python3 combine_overlaps_TEsubfamilies_fullClassCCREs_forR.py <TE subfamily enrichment directory> <TE subfamily-class association file> <min number of overlaps required> <output file basename>
'''
...
for ccre in ccre_types:
	...
	with open(filename, 'w') as o:
		o.write('Subfamily\tClass\tCelltype\tcCRE\tNum_overlaps\n')
