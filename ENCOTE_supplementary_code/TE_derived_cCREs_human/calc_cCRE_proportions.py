'''
Calculate the cCRE proportions (overlapping TEs and not)
Usage: python3 calc_cCRE_proportions.py <input overlap file> <output file>
'''
...
with open(sys.argv[1], 'r') as f:
	...
	for line in f:
		...
		if overlap == 0: #No overlap
			...
			if "DNA" in fields[10]:
				TEclass = "DNA"
			elif "LINE" in fields[10]:
				TEclass = "LINE"
			elif "SINE" in fields[10]:
				TEclass = "SINE"
			elif "LTR" in fields[10]:
				TEclass = "LTR"
			elif "SVA" in fields[10]:
				TEclass = "SVA"
			else:
				TEclass = "Other"
