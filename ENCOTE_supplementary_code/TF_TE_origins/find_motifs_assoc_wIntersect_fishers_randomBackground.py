...
Contingency table:		+intersect	-intersect
	elements_wMotif		    ##		    ##
	elements_without	    ##		    ##
This version is for randomly selected background elements.
Usage: python3 find_motifs_assoc_wIntersect_fishers_randomBackground.py <input stringent motifs file> <input motif annotation file> <input intersect file> <input non-intersect file> <output file>
'''
...
with open(sys.argv[1], 'r') as f:
	...
	for line in f:
		fields = line.rstrip('\n').split('\t')
		top_motif = fields[2]
		stringent_motifs.append(top_motif)
...
with open(sys.argv[4], 'r') as f:
	for line in f:
		...
		if name in di_fishers_cat: #TE in both intersect and non-intersect files, should not happen
			print(name + ' in intersect file ' + sys.argv[3] + ' and non-intersect file ' + sys.argv[4])
