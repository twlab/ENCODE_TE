...
This version uses a length threshold to decide which elements are kept for further processing.
	Removes elements in background if they are shorter than all elements in foreground.
	For randomly selected background.
Usage: python3 calc_foreground_distribution_wLenThresh_randomBackground.py <list of TE subfamilies> <TE consensus motifs file> <directory of overlap/non-overlap files> <directory of alignFastas> <output file basename>
'''
...
for subfamily in list_subfamilies:
	...
	if overlap_file in all_overlap_files:
		...
		di_overlap[subfamily]['min'] = 1000000 #To keep track of minimum length of foreground elements, start at 1Mb (no TE is 1Mb long)
		with open(overlap_dir + overlap_file, 'r') as f:
			for line in f:
				fields = line.rstrip('\n').split('\t')
				name = fields[0] + ':' + fields[1] + '-' + fields[2] + '(' + fields[5] + ')'
				length = int(fields[2]) - int(fields[1])
				di_overlap[subfamily][name] = int(fields[2]) - int(fields[1])
				di_overlap[subfamily]['total'] += 1
				if length < di_overlap[subfamily]['min']:
					di_overlap[subfamily]['min'] = length
	if nonOverlap_file in all_overlap_files:
		...
		with open(overlap_dir + nonOverlap_file, 'r') as f:
			for line in f:
				fields = line.rstrip('\n').split('\t')
				name = fields[0] + ':' + fields[1] + '-' + fields[2] + '(' + fields[5] + ')'
				di_nonOverlap[subfamily][name] = int(fields[2]) - int(fields[1])
					...
					if line.startswith('>'):
						...
						for i in range(len(seq)):
							if seq[i] == '-': #Deletion in element => no coverage at reference base
								...
								if ref[i] == '-': #Insertion in element => no reference base
									...
									coverage.append(1)
						length = len(seq.replace('-', '')) #Length of element is sequence length after removing gaps
						if length < di_overlap[subfamily]['min']: #If length of element is less than the minimum length of foreground elements, skip the element for coverage/subsequent length calculations
							continue #Should not get here with any foreground elements by definition
						if name in di_overlap[subfamily]:
							...
							if 'nonOverlap' not in coverage_di[subfamily]:
								...
								lengths_di[subfamily]['nonOverlap'][name] = str(len(seq.replace('-', '')))
						else: #Element not found (not selected as random background)
							continue
