'''
Summarizes consensus coverage info for significant TE subfamilies per TE class.
Takes a directory of coverage files as input.
Requires file with list of input TE subfamilies (subfamilies to include).
Usage: python3 summarize_consensus_coverage_inputSubfamilies_byClass.py <input coverage directory> <list input TE subfamilies> <TE classes file> <output file>
'''
...
for subfamily in sorted(di):
	...
	if TEclass not in di_results_class:
		di_results_class[TEclass] = []
		for consensus_bin in range(100):
			if consensus_bin in bin_di:
				di_results_class[TEclass].append([stats.mean(bin_di[consensus_bin])])
			else:
				di_results_class[TEclass].append([])
	else:
		for consensus_bin in range(100):
			if consensus_bin in bin_di:
				di_results_class[TEclass][consensus_bin].append(stats.mean(bin_di[consensus_bin]))

with open(sys.argv[-1], 'w') as o:
	o.write('Class\tConsensus_bin\tEnrichment\tUpper\tLower\n')
	for TEclass in di_results_class:
		for consensus_bin in range(100):
			try:
				o.write(TEclass + '\t' + str(consensus_bin) + '\t' + str(stats.mean(di_results_class[TEclass][consensus_bin])) + '\t' +
					str(stats.mean(di_results_class[TEclass][consensus_bin])+stats.stdev(di_results_class[TEclass][consensus_bin])) + '\t' +
					str(stats.mean(di_results_class[TEclass][consensus_bin])-stats.stdev(di_results_class[TEclass][consensus_bin])) + '\n')
			except stats.StatisticsError: #No data in the bin across all TE subfamilies in the family-class
				o.write(TEclass + '\t' + str(consensus_bin) + '\tNA\tNA\tNA\n')
