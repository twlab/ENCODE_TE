'''
Compares shared cCRE vs. lineage cCRE elements in the same TE subfamily for difference in phyloP and phastCons scores.
Only looks at TEs that are orthologous.
Specifically, uses Mann-Whitney U test to test whether shared TE-cCRE and lineage TE-cCRE have the same distributions of phyloP and phastCons scores.
Usage: python3 calc_phylop_phastcons_perSubfamily_compare_orthologTEs.py <cCRE annotation file> <all TE phylop scores file> <all TE phastcons scores file> <output file>
'''
...
for subfamily in list_subfamilies:
	#Group1 is orthologous TE + cCRE in human and mouse
	#Group2 is orthologous TE + cCRE only in one species
	group1_phylop = []
	group1_phastcons = []
	group2_phylop = []
	group2_phastcons = []
			...
			if di[subfamily][coord]['ccre'] == 'both_species':
				if 'phylop_score' in di[subfamily][coord]:
					if di[subfamily][coord]['phylop_cov'] >= cov_thresh:
						group1_phylop.append(di[subfamily][coord]['phylop_score'])
						print(subfamily + '\tphyloP\tgroup1\t' + str(di[subfamily][coord]['phylop_score']) + '\t' + di_info[subfamily]['clade'])
				if 'phastcons_score' in di[subfamily][coord]:
					if di[subfamily][coord]['phastcons_cov'] >= cov_thresh:
						group1_phastcons.append(di[subfamily][coord]['phastcons_score'])
						print(subfamily + '\tphastCons\tgroup1\t' + str(di[subfamily][coord]['phastcons_score']) + '\t' + di_info[subfamily]['clade'])
			else:
				if 'phylop_score' in di[subfamily][coord]:
					if di[subfamily][coord]['phylop_cov'] >= cov_thresh:
						group2_phylop.append(di[subfamily][coord]['phylop_score'])
						print(subfamily + '\tphyloP\tgroup2\t' + str(di[subfamily][coord]['phylop_score']) + '\t' + di_info[subfamily]['clade'])
				if 'phastcons_score' in di[subfamily][coord]:
					if di[subfamily][coord]['phastcons_cov'] >= cov_thresh:
						group2_phastcons.append(di[subfamily][coord]['phastcons_score'])
						print(subfamily + '\tphastCons\tgroup2\t' + str(di[subfamily][coord]['phastcons_score']) + '\t' + di_info[subfamily]['clade'])
	#See if there are enough elements to do the test for phyloP
	if len(group1_phylop) < 5 or len(group2_phylop) < 5:
		continue #Fewer than 5 elements in any group, skip
	di_quartiles[subfamily] = {}
	di_numbers[subfamily] = {}
	#phyloP Mann-Whitney U test
	ustat, pval = mannwhitneyu(group1_phylop, group2_phylop, alternative='two-sided')
	phylop_pvals.append(pval)
	list_tests['phylop'].append(subfamily + '/group1/group2')
	di_quartiles[subfamily]['phylop'] = {}
	di_quartiles[subfamily]['phylop']['group1'] = np.quantile(group1_phylop, [0.25, 0.5, 0.75])
	di_quartiles[subfamily]['phylop']['group2'] = np.quantile(group2_phylop, [0.25, 0.5, 0.75])
	di_numbers[subfamily]['phylop'] = {}
	di_numbers[subfamily]['phylop']['group1'] = len(group1_phylop)
	di_numbers[subfamily]['phylop']['group2'] = len(group2_phylop)
	#See if there are enough elements to do the test for phastCons
	if len(group1_phastcons) < 5 or len(group2_phastcons) < 5:
		continue #Fewer than 5 elements in any group, skip
	#phastCons Mann-Whitney U test
	ustat, pval = mannwhitneyu(group1_phastcons, group2_phastcons, alternative='two-sided')
	phastcons_pvals.append(pval)
	list_tests['phastcons'].append(subfamily + '/group1/group2')
	di_quartiles[subfamily]['phastcons'] = {}
	di_quartiles[subfamily]['phastcons']['group1'] = np.quantile(group1_phastcons, [0.25, 0.5, 0.75])
	di_quartiles[subfamily]['phastcons']['group2'] = np.quantile(group2_phastcons, [0.25, 0.5, 0.75])
	di_numbers[subfamily]['phastcons'] = {}
	di_numbers[subfamily]['phastcons']['group1'] = len(group1_phastcons)
	di_numbers[subfamily]['phastcons']['group2'] = len(group2_phastcons)
...
with open(sys.argv[-1], 'w') as o:
	o.write('Subfamily\tScore_type\tGroup1_number\tGroup2_number\tCall\tq-value\tp-value\tMean_difference\tClade\n')
	#phyloP test result
