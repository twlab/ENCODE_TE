...
Prints t-test results for observed counts based on shuffled counts (background/expectation) for each GWAS parent term.
Usage: python3 calc_enrichment_observed_vs_shuffled_negative.py <observed counts file> <directory of shuffled counts files> <output file>
'''
...
for filename in list_files:
	with open(dir_shuffled + filename, 'r') as f:
		...
		for line in f:
			...
			di[parent_term]['ccre'].append(float(fields[1]))
			di[parent_term]['nonTE'].append(float(fields[2]))
			di[parent_term]['TE'].append(float(fields[3]))
			di[parent_term]['human_nonTE'].append(float(fields[4]))
			di[parent_term]['human_TE'].append(float(fields[5]))
...
with open(sys.argv[1], 'r') as f:
	...
	for line in f:
		fields = line.rstrip('\n').split('\t')
		parent_term = fields[0]
		observed[parent_term] = {'ccre': float(fields[1]), 'nonTE': float(fields[2]), 'TE': float(fields[3]), 'human_nonTE': float(fields[4]), 'human_TE': float(fields[5])}
